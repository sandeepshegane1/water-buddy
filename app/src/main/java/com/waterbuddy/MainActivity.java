...
public class MainActivity extends Activity {
  ...
  void build(){
    ...
    TextView title=label("💧 Water Buddy",30); title.setTextColor(blue); page.addView(title); TextView latest=label("Your cheerful hydration companion",17); page.addView(latest); latest.setOnClickListener(v -> { ReminderReceiver.showReminder(this); });
    ...
    voice=new Switch(this); voice.setText("🔊 Cute baby voice reminders"); voice.setTextSize(17); voice.setChecked(prefs.getBoolean("voice",true)); voice.setPadding(8,24,8,24); page.addView(voice);
  ...
  void addGlass(){String today=new SimpleDateFormat("yyyy-MM-dd",Locale.US).format(new Date()); if(!today.equals(prefs.getString("date","")))prefs.edit().putString("date",today).putInt("glasses",0).commit(); int g=prefs.getInt("glasses",0)+1; prefs.edit().putInt("glasses",g).commit(); total.setText(g+" glasses today"); message.setText(g>=8?"✅ Goal reached!":"Keep going!"); }
  void refresh(){String today=new SimpleDateFormat("yyyy-MM-dd",Locale.US).format(new Date()); if(!today.equals(prefs.getString("date","")))prefs.edit().putString("date",today).putInt("glasses",0).commit(); int g=prefs.getInt("glasses",0); total.setText(g+" glasses today"); message.setText(g>=8?"✅ Goal reached!":""); }
  ...
  void requestPermission(){if(Build.VERSION.SDK_INT>=33&&checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED)requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS},1);}
  void schedule(){AlarmManager a=getSystemService(AlarmManager.class); PendingIntent p=PendingIntent.getBroadcast(this,1,new Intent(this,ReminderReceiver.class).setAction("REMIND"),PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE); a.setAndAllowWhileIdle(AlarmManager.ELAPSED_REALTIME_WAKEUP,SystemClock.elapsedRealtime()+3600000,p);}
}
